...
export function useInitiatePayment() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Call Edge Function to initiate Flutterwave payment
      const { data: flutterwaveResult, error: fnError } = await supabase.functions.invoke('initiate-payment', {
      ...
      if (flutterwaveResult?.error) {
        await supabase.from('payments').delete().eq('reference', reference);
        throw new Error(flutterwaveResult.error);
      }
