...
export function TestStripePayment() {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-card border rounded-lg shadow-lg p-8 space-y-6">
        <div className="text-center">
          <div className="w-16 h-16 rounded-full bg-[#635BFF] flex items-center justify-center mx-auto mb-4">
            <CreditCard className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-2xl font-bold">Test Stripe Payment</h1>
          <p className="text-muted-foreground mt-2">
            This page lets you test the Stripe Checkout flow without needing a winning auction bid.
          </p>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-900">
            Use test card <strong>4242 4242 4242 4242</strong> with any future expiry and any CVC.
          </p>
        </div>
        ...
        >
          {loading ? (
            ...
            "Pay US$10.00 with Stripe"
          )}
        </Button>

        <div className="flex items-center justify-center gap-1 text-xs text-muted-foreground">
          <Lock className="w-3 h-3" /><span>Secured by Stripe (Test Mode)</span>
        </div>
      </div>
    </div>
  );
